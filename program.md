...
**Tighter CI (optional).** If the claim needs a 95% confidence interval on Δ (not just a p-value — e.g. for publishing a delta figure), continue to **R=29 reruns per variant** and use a Newcombe/Wilson score-difference CI.

**Quick-reference table** (generated by `eval/rerun_analysis.py`):

| Confidence level | Protocol | Reruns per variant | Min detectable lift |
|---|---|---:|---|
| Directional screen only | 4-run screen, no statistical claim | 4 | ~+4 tasks |
| 95% p-value (one-sided) | Two-prop z-test | 10 | +2 tasks |
| 95% p-value (two-sided) | Two-prop z-test | 15 | +2 tasks |
| 95% CI for Δ | Newcombe/Wilson diff CI | 29 | +2 tasks |

**Lite eval is NOT a statistical claim.** The 20-task curated lite eval remains the inner-loop dev signal — "is canary still 4/4?", "did the dispute_calculator cluster move?" — and nothing more. Do not report "lite improved by 2 tasks" as evidence a change works; lite has even higher per-task noise and covers only one-fifth of the benchmark. Full-eval rerun statistics per the table above are the only way to publish an accuracy claim.

With concurrency=12 a single full eval is ~16 min, so Stage A (8 evals) is ~2 hours and a full Stage B (30 evals) is ~8 hours — budget accordingly before committing to a confirmed claim.
